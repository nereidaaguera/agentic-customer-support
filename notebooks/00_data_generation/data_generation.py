...
def get_table_count(table_name: str) -> int:
    ...
    return spark.sql(f"SELECT COUNT(*) FROM telco_customer_support_{env}.bronze.{table_name}").collect()[0][0]
...
FROM telco_customer_support_{env}.bronze.customers c
LEFT JOIN telco_customer_support_{env}.bronze.subscriptions s ON c.customer_id = s.customer_id
...
FROM telco_customer_support_{env}.bronze.billing
...
FROM telco_customer_support_{env}.bronze.devices d
...
FROM telco_customer_support_{env}.bronze.knowledge_base
...
FROM telco_customer_support_{env}.bronze.support_tickets
